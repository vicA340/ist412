...
(all refactoring was completed by Christian Chernock)
    ...
    Fixed 500 caused by registering with a username that already exists
    ...
    Fixed password encryption process so that the web security configuration and user registration agree on the password encryption method for user passwords.
    ...
    The dashboard controller was separated from the MainController class and put into its own dedicated controller class

    • This was accomplished by creating a DashboardController class to handle dashboard functions.

    Fixed redirect issue for users attempting to view loans when not logged in (via typing the url).
    ...
    Cleaned up the Loan use case implementations

    • Got rid of the LoanStatus class and added a check method to the Loan class. • Combined the LoanService and LoanServiceImpl classes into one class

    Cleaned up the WebSecurityConfiguration class.
